...
namespace neural
{
    ...
    Network::Network() : mSigmoid(-10.f, 10.f, 256)
    {
        mTrainThread = std::thread([&](){
            ...
            while (!mStopping)
            {
                if (mAsyncTrainDataReady.load() != nullptr)
                {
                    shuffledData.clear();
                    for (auto& result : mAsyncTrainData)
                        shuffledData.emplace_back(&result);
                    mAsyncTrainDataReady.store(nullptr);
                    miniBatchSize = mAsyncMiniBatchSize.load();
                    learningRate = mAsyncLearningRate.load();
                    log = mAsyncLog;
                }
                if (!shuffledData.empty())
                    trainEpoch(shuffledData, miniBatchSize, learningRate, log, epoch++);
            }
        });
    }
    ...
    void Network::trainEpoch(std::vector<const Result*>& shuffledData, int miniBatchSize, Value learningRate, bool log, int epochNr)
    {
        ...
        {
            std::string output;
            output.append("epoch " + std::to_string(epochNr));
            output.append(" input (");
    ...
    void Network::trainAsync(const std::vector<Result>& data, int miniBatchSize, Value learningRate, bool log)
    {
        mAsyncTrainData = data;
        mAsyncMiniBatchSize = miniBatchSize;
        mAsyncLearningRate = learningRate;
        mAsyncLog = log;
        mAsyncTrainDataReady.store(&mAsyncTrainData);
    }
	...
	void Network::train(const std::vector<const Result*>& data, Value learningRate)
	{
		...
		for (auto& result : data)
		{
			processInputValues(result->mInputs);
    ...
    std::vector<Value> Network::process(const std::vector<Value>& inputValues)
    {
        std::lock_guard<std::mutex> lock(mMutex);

        processInputValues(inputValues);
        std::vector<Value> result = getOutputValues();
        return result;
    }
    ...
    std::vector<Value> Network::getErrorMargins(const std::vector<Result>& data)
    {
        ...
        {
            processInputValues(data[i].mInputs);
    ...
    void Network::processInputValues(const std::vector<Value>& inputValues)
    {
